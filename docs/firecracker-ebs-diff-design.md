...
> 状态：**已实现并完成真机验证**。方案 C 持久 EBS、Diff 快照、merged base 转正、
> resume 网络修复和并发限流均已合入；本文同时保留设计演进、失败路线和后续修复证据。
> Spot 中断后的自动跨节点恢复仍未闭环，当前运行边界以 [`../README.md`](../README.md)
> 为准。
>
> 🔴 **2026-08-13 补充（x86 r8i 真机）**：数据面已按本文思路拆成 On-Demand + Spot 两个
> 节点组（详见
> [OD/Spot 双池 + 预热池 + 预打包运行环境 真机测试报告](OD-Spot双池-节点预热池-预打包运行环境-真机测试报告-2026-08-13.md)），
> 但**控制面 `_pick_node()` 只按 `free_mem_mib` 排序，不看节点的 `capacity-type` label**，
> 实测 5/5 个沙盒全落 OD 节点。也就是说本文假设的"沙盒常态跑在 Spot 上、被回收时疏散到
> OD"这条路径**目前还没有调度侧入口**，Spot 池开了不会被用到。这是方案 C 闭环的下一个
> 前置项。
...
> 零性能损失**。同一逻辑也决定了 x86 的默认机型：x86 路径用支持嵌套虚拟化的 `r8i.*`
> （默认 `r8i.8xlarge`，无本地 NVMe，状态全落持久 EBS）；`i7i.*` 仍可选，但它的本地
> NVMe 是 instance store，方案 C 下同样用不上。Graviton 路径固定使用 `c6g.metal`。
