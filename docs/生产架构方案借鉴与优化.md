...
| §8 | **可观测性** | 30+ 指标**分层清单** + 5 类告警矩阵 + **高基数治理策略** | **P1 已落地**：低基数指标、结构化日志、5 类告警、8 面板 Dashboard、SHA-256 校验、AMP + AMG | ✅ 核心链路已通过真实 AWS E2E；集中日志、tracing 和通知接收器仍待补 | **DONE / P2** | 中 |
...
## 8. ✅ 可观测性：P1 核心链路已落地并通过真实 AWS E2E
...
仍未覆盖：CloudWatch Logs 集中日志、OpenTelemetry tracing、Alertmanager 外部通知接收器，
以及 AMG workspace/datasource 的全自动生命周期管理。
...
下一阶段应聚焦 P2 运维闭环，而不是继续堆指标：

1. Alertmanager 接入 SNS/PagerDuty/飞书等真实通知渠道并演练升级策略
2. 日志集中进入 CloudWatch Logs 或兼容后端，按 correlation ID 检索
3. 用 OpenTelemetry 补跨 control-plane/node-agent 的 tracing
4. 将 AMG datasource/Dashboard 纳入可重复自动化，同时避免长期 API token
...
| ~~快照完整性校验~~ | **DONE** | §8 |
| 集中日志 + tracing + Alertmanager 外部通知 | P2 | §8 |
