...
> 整理日期：2026-08-04；实施更新：2026-08-11
...
**一句话总结**：控制面/数据面已拆分并通过 i7i 真机 E2E。控制面、LiteLLM 和系统 Pod 固定在小型 On-Demand Graviton system 节点；Firecracker 数据面独占带 taint 的 sandbox 节点。未来只有数据面可引入 Spot，但在中断消费、快照疏散和跨节点恢复闭环前仍应使用 On-Demand。其余优先项仍是 S3 加固、完整成本口径和万级扫描 GSI。
...
- `scan_once` 仍通过 `db.list_by_states(["running"])` 扫描 sandboxes 表；该表没有 state GSI，
  万级实例下仍需改成索引查询。
- 本地测试 50/50 通过；AWS `i7i.8xlarge` 真机扩展 E2E A0-A6 全部通过，
...
4. `fc_snapshot_verify_total` 对应的**快照完整性校验本仓库完全没有**（当前只有 `os.path.exists`）——考虑到快照里是用户全部状态，这值得单独列一项
