...
|---|---|---|---|---|---|---|
| §2 | **机型与节点池** | `i7i.8xlarge` 数据面 + 独立 system 节点 | **已实施控制面/数据面分离**：On-Demand Graviton system；sandbox 可选 `c6g.metal` 或 i7i | ✅ i7i、异构镜像、调度隔离和控制面故障转移已通过真机 E2E | **分离已验证，Spot 恢复待闭环** | 中 |
...
| §10 | **架构叙事** | "沙盒实例不是 Pod"——一个刻意选择及其理由 | **已完成**：中英文 README 已说明平台 Pod 与用户 microVM 的调度、网络、存储、生命周期及可观测性边界 | ✅ 已落地；新 VMM 已脱离 node-agent Pod cgroup，同时保留 EBS 同 AZ边界 | **DONE** | 极小 |
...
节点。未来只有数据面可引入 Spot，但在中断消费、快照疏散和跨节点恢复闭环前仍应使用
On-Demand。其余优先项仍是 S3 加固、完整成本口径和万级扫描 GSI。
...
```
terraform/phase3/main.tf   arm64 → c6g.metal / amd64 → i7i.8xlarge（默认）
terraform/phase3/main.tf   amd64 可用 sandbox_instance_type 覆盖为任意 i7i 规格
```
...
`cpu_options.nested_virtualization = "enabled"`，不是只替换实例名。实例校验会拒绝非 i7i 的
amd64 规格，避免在不支持嵌套虚拟化的 x86 机型上创建节点后才发现 `/dev/kvm` 缺失。

改造前的 Graviton 与旧 x86 metal 路径都没有本地 NVMe，而本仓库自己的实测报告早已把它定为核心痛点。
当前 i7i 路径提供本地 NVMe，但本次兼容性改造仍以 EBS 为权威状态盘，尚未把 instance store
接入快照缓存：
...
4. 不做单向切换：部署者明确选择 Graviton 或 x86；Graviton 继续走 `c6g.metal`，x86 走 i7i
...
其余：无 Secrets Manager / External Secrets（密钥经 Terraform `-var` 传入，`litellm.tf:107` 注释自陈"生产应从 Secrets Manager 取"）；**IAM 资源范围过宽** —— action 已逐条枚举（做得对），但 `ec2:DescribeInstances` / `ssm:SendCommand` / ECR 拉取类均为 `Resource = ["*"]`（`stage2-control-plane/main.tf:269-280, 329-335`），其中 `ssm:SendCommand` on `*` 意味着可对账号内任意实例下发命令；jailer 与逐 VMM systemd cgroup 已于 2026-09-02 落地，但宿主路径仍需真机/FIS 复验；无 CloudTrail、无 EKS 审计日志组、无租户间 NetworkPolicy。
...
**其余（Secrets Manager / IAM 资源范围收窄 / NetworkPolicy / CloudTrail）：P2**，
逐项跟进即可；jailer/逐 VMM cgroup 已实现，当前剩余的是升级后真机复验。
...
直接管理 Firecracker 进程；每个 sandbox 不对应 Pod、PVC 或 Service。路线 A 后续增加了
轻量 `FirecrackerSandbox` CRD 作为期望态/API 身份，但 VMM 仍不是 Pod。2026-08-12
已在 [`README.md`](../README.md) 与 [`README.en.md`](../README.en.md) 增加完整说明和对照表，
明确 Kubernetes 管平台组件，而本项目控制面负责 microVM 的放置与生命周期。

2026-09-02 后续改造已把新 VMM 交给宿主 systemd+jailer 管理，脱离 node-agent Pod
cgroup；首次从旧版本迁移时，旧 VMM 仍必须先安全疏散。README 同时记录了 EBS 同 AZ
约束和跨 AZ 仍依赖 S3，避免把逻辑解耦描述成宿主故障下的完全独立。
...
| Secrets Manager / IAM 资源范围收窄 / 租户 NetworkPolicy / CloudTrail | P2 | §4 |
...
- jailer 默认关（既有 P2）→ 2026-09-02 已改为 host-systemd+jailer，待真机复验（§4）
