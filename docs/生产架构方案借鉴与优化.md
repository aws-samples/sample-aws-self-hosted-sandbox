...
> 整理日期：2026-08-04；实施更新：2026-08-10
...
|---|---|---|---|---|---|---|
| §2 | **机型选型** | `i7i.8xlarge` 虚拟化实例（嵌套虚拟化 + 本地 NVMe），5 机型对比表 | **已实施双架构选择**：Graviton=`c6g.metal`，x86=`i7i.8xlarge`（可覆盖任意 `i7i.*`） | ✅ `.metal` 不再是 x86 必需条件；i7i 兼容性 E2E 已通过 | **兼容性已验证，待性能基准** | 中 |
...
**一句话总结**：空闲检测第一阶段已经落地（§6）；接下来最该做的是 ① S3 快照桶加固（§4）；② 把"沙盒不是 Pod"和完整成本口径写进 README（§9/§7）；③ 为万级扫描增加 state GSI。机型路径已完成第一阶段改造并通过 i7i 真机兼容性 E2E：部署时由用户选择 Graviton 或 x86，前者使用 `c6g.metal`，后者默认 `i7i.8xlarge` 并允许任意 i7i 规格；下一步是完成与 c6g.metal 的性能基准。
...
当前 Terraform 对 x86 节点的 Launch Template 显式设置
...
**已实施范围**：`terraform/phase3/main.tf` 使用 `node_arch` / `sandbox_instance_type` 表达两条路径，并为 i7i 设置 `cpu_options.nested_virtualization`；rootfs 和控制面镜像构建支持 `linux/amd64`；节点引导只把 Amazon EBS 识别为权威状态盘，避免误格式化 i7i 本地 NVMe。当前仍维持 EBS 权威模型，本地 NVMe 快照缓存属于后续性能优化，不与本次实例兼容改造混做。

**真机验证结果**：2026-08-10 在 `us-east-1b` 使用 `i7i.8xlarge` 完成
create/exec/suspend/resume/destroy/auth E2E，宿主 `/dev/kvm` 与 amd64 guest 均正常。
详见 [`i7i-e2e-test-report-2026-08-10.md`](i7i-e2e-test-report-2026-08-10.md)。
