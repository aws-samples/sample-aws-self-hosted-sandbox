...
>
> **状态更新（2026-08-13）**：本文"节点发现靠环境变量硬编码"这项虽已换成 DynamoDB
> 心跳注册表，但**调度策略仍是最朴素的 `free_mem_mib` 降序**（`drivers/firecracker.py`
> `_pick_node()`）——不看 `capacity-type`、不看 rootfs 模板是否齐备、不做亲和/反亲和。
> 真机后果已实测到两条：① 数据面拆出 Spot 节点组后沙盒仍 5/5 全落 On-Demand 节点；
> ② EC2 预热池换出的节点缺命名 rootfs 模板时照样被选中，`image=claude-code` 静默降级
> 成 min。**"调度器只有一条打分维度"应视为本文之后新增的一项 P0 gap**。证据见
> [`OD-Spot双池-节点预热池-预打包运行环境-真机测试报告-2026-08-13.md`](OD-Spot双池-节点预热池-预打包运行环境-真机测试报告-2026-08-13.md)
> §6、§8。
