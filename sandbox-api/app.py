...
# 池分离总开关:关掉(0)则 pool 一律 None(不限池,回到原行为),便于单池集群或回滚。
...
def create_sandbox(body: dict) -> tuple[int, dict]:
    ...
    record: dict = {
        ...
        # M2:期望池(记录下来供观测/重调度参考;实际落点见 driver 返回的 node)。
        "pool":             pool or "",
    ...
    try:
        ...
        # M2:暖池预热的 VM 池归属不确定(可能在任意池),若请求了明确 pool 则跳过暖池,
        # 走冷建以保证落到目标池。未指定 pool(None)时仍可用暖池(维持秒级 create)。
        use_warm = wants_default and pool is None
        claimed = _warm_pool.claim(sid, spec) if use_warm else False
        if not claimed:
            driver_fields = _driver.create(sid, spec, epoch=1, pool=pool)
