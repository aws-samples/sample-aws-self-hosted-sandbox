...
eksctl delete cluster --name claude-sbx --region $AWS_REGION
...
| R7 | ~~任意端口暴露规模~~ **已确认:绝大多数 HTTP(80/443/8080),少数其他端口,且非每个沙盒都暴露;UDP 暂不需要** | 方案已收敛为通配符子域名 + 共享 NLB + ingress-nginx(见 3.4) | ✅ 已确认 |
| R8 | **已确认:租户互不可信** —— 主层必须 microVM(不可用 Sysbox/容器),且 **Bedrock 凭据绝不能进沙盒** | 决定隔离强度 + 凭据隔离设计 | ✅ microVM 已是当前设计;凭据隔离见 1.8(宿主侧出口代理 / 每租户短期凭据),**POC 第二阶段必验** |
| R9 | 沙盒生命周期:**已确认先测 24×7 长驻,snapshot 放 Phase 2** | 影响压测与成本杠杆排期 | ✅ Phase 1/3 验 24×7;快照恢复(成本核心杠杆)放 Phase 2/5 |
